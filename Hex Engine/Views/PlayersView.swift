...
struct PlayersView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ForEach (world.playerTurnSequence, id: \.self) { playerID in
                    Text("\(self.getPlayerName(playerID)) (\(playerID))").background( playerID == self.world.currentPlayer?.id ? Color.blue : Color.clear)
                }
            }
